...
class ConjugateGradient(Optimizer):
    ...
    def __init__(self, beta_type="DaiYuan", orth_value=np.inf, linesearch=None, *args, **kwargs):
        """
    ...
    def run(self, problem, * ,initial_point=None, reuselinesearch=False)-> OptimizerResult:
        """
        ...
        def _Pgrad(_x):
            return problem.preconditioner(_x, gradient(_x))
        # Initial descent direction is the negative gradient
        desc_dir = -Pgrad
        
        self._initialize_log(
            optimizer_parameters={
                "beta_rule": self._beta_type,
        ...
        while True:
            ...
            self._add_log_entry(iteration=iter, point=x, cost=cost, gradient_norm=gradnorm)

            stopping_criterion= self._check_stopping_criterion(start_time=time0, gradient_norm=gradnorm, iteration=iter + 1, step_size=stepsize)
            ...
            # Execute line search
            stepsize, newx = linesearch.search(objective, manifold, x, desc_dir, cost, df0, _Pgrad)

            # Compute the new cost-related quantities for newx
            newcost = objective(newx)
            newgrad = gradient(newx)
            newgradnorm = manifold.norm(newx, newgrad)
            ...
            if abs(orth_grads) >= self._orth_value:
                ...
                if self._beta_type == "DaiYuan":
                    diff = newgrad - oldgrad
                    beta = newgradPnewgrad / manifold.inner_product(newx, diff, desc_dir)
                elif self._beta_type == "PolakRibiere":
                    diff = newgrad - oldgrad
                    ip_diff = manifold.inner_product(newx, Pnewgrad, diff)
                    beta = ip_diff / gradPgrad
                elif self._beta_type == "Hybrid1":
                    diff = newgrad - oldgrad
                    beta_DY = newgradPnewgrad / manifold.inner_product(newx, diff, desc_dir)
                    ip_diff = manifold.inner_product(newx, Pnewgrad, diff)
                    try:
                        beta_HS = ip_diff / manifold.inner_product(newx, diff, desc_dir)
                    except ZeroDivisionError:
                        beta_HS = 1
                    beta = max(0, min(beta_DY, beta_HS))
                elif self._beta_type == "Hybrid2":
                    diff = newgrad - oldgrad
                    beta_DY = newgradPnewgrad / manifold.inner_product(newx, diff, desc_dir)
                    ip_diff = manifold.inner_product(newx, Pnewgrad, diff)
                    try:
...
class LineSearchWolfe:
    ...
    def search(self, objective, manifold, x, d, f0, df0, gradient):
        ...
        def phi(alpha):
            fc[0] += 1
            return objective(manifold.retraction(x, alpha * d))

        def derphi(alpha):
            newx = manifold.retraction(x, alpha * d)
            newd = manifold.transport(x, newx, d)
            gc[0] += 1
            gval[0] = gradient(newx)  # store for later use
            gval_alpha[0] = alpha
            return manifold.inner_product(newx, gval[0], newd)

        gfk = gradient(x)
        derphi0 = manifold.inner_product(x, gfk, d)

        stepsize = _scalar_search_wolfe(phi, derphi, self.c1, self.c2, maxiter=100)
        if stepsize is None:
            stepsize = 1e-6
        
        newx = manifold.retraction(x, stepsize * d)
